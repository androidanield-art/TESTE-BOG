...
export const downloadPPT = (data: PresentationData, clientName: string) => {
  ...
  data.slides.forEach((slide, index) => {
    ...
    if (slide.type === 'cover') {
      s.addText("PROPOSTA ESTRATÉGICA", { x: 0.6, y: 1.8, fontSize: 11, color: ACCENT_COLOR, bold: true, charSpacing: 4 });
      s.addText("ECOSSISTEMA", { x: 0.5, y: 2.3, w: 9.0, fontSize: 64, color: TEXT_COLOR, bold: true, fontFace: "Arial Black", italic: true });
      s.addText("DE GROWTH", { x: 0.5, y: 3.2, w: 9.0, fontSize: 64, color: SUBTEXT_COLOR, bold: true, fontFace: "Arial Black", italic: true });
      if (slide.subtitle) {
        s.addText(slide.subtitle, { x: 0.8, y: 4.5, w: 6.5, fontSize: 18, color: SUBTEXT_COLOR, italic: true });
        s.addShape(pres.ShapeType.line, { x: 0.6, y: 4.5, w: 0, h: 0.8, line: { color: ACCENT_COLOR, width: 4 } });
      }
    } 
    
    else if (slide.type === 'content') {
      s.addText(slide.title, { x: 0.5, y: 1.5, w: 3.2, fontSize: 28, color: ACCENT_COLOR, bold: true, fontFace: "Arial Black", italic: true });
      if (slide.subtitle) s.addText(slide.subtitle, { x: 0.5, y: 2.8, w: 3.2, fontSize: 13, color: SUBTEXT_COLOR, italic: true });
      s.addShape(pres.ShapeType.line, { x: 4.2, y: 1.2, w: 0, h: 4.2, line: { color: BORDER_COLOR, width: 1.5 } });
      slide.content.forEach((point, i) => {
        s.addText(point, { x: 4.6, y: 1.5 + (i * 0.8), w: 4.8, fontSize: 16, color: TEXT_COLOR, bullet: { code: "25CF" } });
      });
    }
      ...
      CATEGORIES.forEach(cat => {
        ...
        if (items && items.length > 0) {
            ...
            items.forEach((item, k) => {
                s.addText(item.name, { x: colX + 0.1, y: 2.35 + (k * 0.4), w: 2.7, fontSize: 10, color: TEXT_COLOR, bold: true, bullet: { code: "25AA" } });
            });
            colX += 3.15;
        }
      });
  ...
  pres.writeFile({ fileName: `APRESENTACAO_${clientName.replace(/\s+/g, '_').toUpperCase()}.pptx` });
};